...
|v|[string](http://www.w3.org/TR/xmlschema-2/#string)|The value of the tile source tag. <ul><li>A vertical bar "`\|`" can be used to specify multiple values.</li><li>An asterisk "`*`" serves as wildcard character.</li><li>A minus sign "`-`" matches when either key is not present or excludes the other values after "`\|`". It never works alone.</li><li>A tilde "`~`" matches matches when either key is not present or if the map element does not have a tag with the specified key.</li>|yes|
