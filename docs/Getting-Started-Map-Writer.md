...
- There is a potential issue with tile grid artifacts inside areas at zoom levels 12-13, specially if they have stroke or semi-transparent fill. To solve this you can try non-transparent fill without stroke or adding `force-polygon-line="true"` in the tag-mapping or a different zoom interval:
```bash
