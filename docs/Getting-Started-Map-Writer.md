...
|`preferred-languages`|If not specified or preferred-languages=- only the default language with no tag will be written to the file. If only one language or multiple comma separated languages are specified, the default tagless language will be written, followed by the specified languages (if present and if different than the default). If preferred-languages=* all languages found are written.|language code as as defined in ISO 639-1 or ISO 639-2|(blank)|
