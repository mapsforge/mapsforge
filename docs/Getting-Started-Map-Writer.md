...
|`preferred-languages`|If not specified or '-' only the default language with no tag will be written to the file. If only one language is specified, it will be written if its tag is found, otherwise the default language will be written. If multiple comma separated languages are specified, the default language will be written, followed by the specified languages (if present and if different than the default). If '*' all languages found are written.|language code as as defined in ISO 639-1 or ISO 639-2|(blank)|
