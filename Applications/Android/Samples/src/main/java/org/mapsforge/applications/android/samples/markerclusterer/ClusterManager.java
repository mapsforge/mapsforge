...
public class ClusterManager<T extends GeoItem> implements Observer, SelectionHandler<T> {
	 ...
	 */
	protected List<MarkerBitmap> mMarkerIconBmps = Collections
    ...
    /**
     * You might like to set the Toast from external, in order to make sure that onl a single Toast
     * is showing up.
	 ...
	 */
	protected List<Cluster<T>> mClusters = Collections
	...
	public synchronized List<T> getAllItems() {
		...
		synchronized ( mClusters) {
			for (Cluster<T> mCluster : mClusters ) {
...
//		Log.d(TAG,"setSelectedItem(Selecti... (super) is called" );
...
//			}
//		}
		synchronized (this.mMarkerIconBmps) {
...
//					Log.i(TAG,"public synchronized void addItem(T item) {... (5)");
					...
					if (mCluster.getItems().size() == 0)
						throw new IllegalArgumentException("mCluster.getItems().size() == 0");
				...
				} // END: for (Cluster<T> mCluster : mClusters) {
				// No cluster contain the marker, create a new cluster.
				mClusters.add(createCluster(item));
			} // END: synchronized (mClusters) {
		} else {
	...
	protected synchronized BoundingBox getCurBounds() {
		if ( currBoundingBox == null ) {
			...
			if ( mMapView.getWidth() <= 0 || mMapView.getHeight() <= 0) {
				throw new IllegalArgumentException(" mMapView.getWidth() <= 0 " +
						"|| mMapView.getHeight() <= 0 " 
...
//			Log.e(TAG, " se_.latitude => " + se_.latitude + " se_.longitude => " + se_.longitude );
				...
				synchronized (mClusters) {
					...
					for (Cluster<T> mCluster : mClusters) {
						...
						synchronized (mLeftItems) {
							...
							mLeftItems.addAll(mCluster.getItems());
	...
	public synchronized void destroyGeoClusterer() {
		...
		/** Clustered object list. */
		// Log.w(TAG,"synchronized (mLeftItems) {... in destroyGeoClusterer() {....");
		synchronized (mLeftItems) {
			mLeftItems.clear();
		}
