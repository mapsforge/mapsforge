...
public class ZipXmlThemeResourceProviderTest {
    ...
    public void openZip() throws IOException {
        final ZipInputStream zis = new ZipInputStream(new BufferedInputStream(ZipXmlThemeResourceProviderTest.class.getResourceAsStream("/xmlthemetest.zip")));
        ...
        Assert.assertEquals(8, zts.getSize());

        final List<String> xmlThemes = new ArrayList<>();
        for (ZipXmlThemeResourceProvider.XmlTheme theme : zts.getXmlThemes()) {
            xmlThemes.add(theme.toString());
        }
