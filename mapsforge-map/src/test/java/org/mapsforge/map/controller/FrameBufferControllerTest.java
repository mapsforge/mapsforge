...
public class FrameBufferControllerTest {
	static class DummyFrameBuffer implements FrameBufferInterface {
		Dimension dimension;
		float lastDiffX;
		float lastDiffY;
	...
	public void frameBufferControllerTest() {
		DummyFrameBuffer dummyFrameBuffer = new DummyFrameBuffer();
		Model model = new Model();
		new FrameBufferController(dummyFrameBuffer, model);
		...
		Assert.assertEquals(new Dimension(100, 200), dummyFrameBuffer.lastMapViewDimension);
		Assert.assertEquals(new Dimension(150, 300), dummyFrameBuffer.dimension);

		model.mapViewPosition.setMapPosition(new MapPosition(new GeoPoint(0, 180), (byte) 1));
		Assert.assertEquals(-256, dummyFrameBuffer.lastDiffX, 0);
		Assert.assertEquals(0, dummyFrameBuffer.lastDiffY, 0);
		Assert.assertEquals(2, dummyFrameBuffer.lastScaleFactor, 0);
	}
}
