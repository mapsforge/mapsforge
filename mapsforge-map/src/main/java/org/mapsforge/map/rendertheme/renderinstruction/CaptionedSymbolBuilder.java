 ...
 */
public class CaptionedSymbolBuilder extends RenderInstructionBuilder {
	...
	float fontSize;
	float gap;
	boolean renderCaptionless = true;
	final Paint stroke;
	TextKey textKey;

	public CaptionedSymbolBuilder(GraphicFactory graphicFactory, DisplayModel displayModel, String elementName,
	 ...
	 */
	public CaptionedSymbol build() {
		return new CaptionedSymbol(this);
	}
	...
	private void extractValues(GraphicFactory graphicFactory, DisplayModel displayModel, String elementName,
		...
		for (int i = 0; i < attributes.getLength(); ++i) {
			...
			if (K.equals(name)) {
				...
				this.dy = XmlUtils.parseNonNegativeFloat(name, value) * displayModel.getScaleFactor();
			...
			} else if (RENDER_CAPTIONLESS.equals(name)) {
				this.renderCaptionless = Boolean.parseBoolean(value);
