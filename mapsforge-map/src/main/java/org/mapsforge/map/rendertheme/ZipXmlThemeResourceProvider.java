...
public class ZipXmlThemeResourceProvider implements XmlThemeResourceProvider {
    ...
    public ZipXmlThemeResourceProvider(ZipInputStream zipInputStream, int maxResourceSizeToCache) throws IOException {
        ...
        try {
            ...
            while ((zipEntry = zipInputStream.getNextEntry()) != null) {
                ...
                String fileName = zipNameToFileName(zipEntry.getName());
     ...
     *
     * This method is useful e.g. to find out which Xml Themes are available across multiple Zip files without actually have to read them all into memory.
     *
     * @param zipInputStream zip stream to read resources from
     * @return the XML theme paths in the archive
     * @throws IOException if a problem occurs reading the stream     */
    public static List<String> getXmlThemesIn(ZipInputStream zipInputStream) throws IOException {
        if (zipInputStream == null) {
    ...
    private static String zipNameToFileName(String zipName) {
